  ...
  const cardContent = useMemo(() => (
    ...
    >
      ...
      <div className={`w-auto sm:w-[100px] flex items-center justify-end gap-1 flex-shrink-0 transition-opacity duration-200 ${liked ? "opacity-100" : "opacity-100 sm:opacity-0 sm:group-hover:opacity-100"}`}>
        ...
        <button onClick={handleFav} className={`p-1 sm:p-1.5 transition-all rounded-full ${liked ? "text-red-500" : "text-gray-500 hover:text-red-400 active:text-red-400"}`} title={liked ? "Bỏ yêu thích" : "Yêu thích"}>
          <HiHeart className={`text-[11px] sm:text-sm lg:text-base ${liked ? "drop-shadow-[0_0_4px_rgba(239,68,68,0.4)]" : ""}`} />
        </button>
        <button onClick={handleMenu} className="p-1 sm:p-1.5 text-gray-500 hover:text-white active:text-white transition-colors rounded-full" title="Thêm tùy chọn">
          <HiDotsHorizontal className="text-[11px] sm:text-sm lg:text-base" />
        </button>
      </div>
    </div>
  ), [song.id, song.title, song.artist, song.cover, song.duration, isActive, isActiveAndPlaying, liked]);
