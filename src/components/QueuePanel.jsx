      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-white/5">
          <div className="flex items-center gap-2">
            <HiQueueList className="text-neon text-base" />
            <h2 className="text-base font-bold text-white">Danh sách phát</h2>
          </div>
        ...
        {currentSong && (
          <div className="px-4 py-3 bg-neon/5 border-b border-white/5">
            <p className="text-[10px] font-bold text-neon uppercase tracking-widest mb-2">Đang phát</p>
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-full overflow-hidden flex-shrink-0 border border-neon/20 ${isPlaying ? "animate-spin-slow" : ""}`}>
                <img src={currentSong.cover} alt={currentSong.title} className="w-full h-full object-cover" />
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-neon truncate">{currentSong.title}</p>
        ...
        {manualQueue.length > 0 && (
          <div className="border-b border-white/5">
            <p className="px-4 pt-3 pb-1 text-[10px] font-bold text-neon/70 uppercase tracking-widest">
...
function QueueItem({ song, index, onClick }) {
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-2.5 px-2 py-1.5 rounded-lg text-left hover:bg-white/5 group transition-colors"
    >
      <span className="text-[10px] text-gray-600 font-mono w-4 text-right flex-shrink-0">{index}</span>
      <img src={song.cover} alt={song.title} className="w-8 h-8 rounded-md object-cover flex-shrink-0" />
      <div className="min-w-0 flex-1">
        <p className="text-xs text-gray-300 truncate group-hover:text-white">{song.title}</p>
        <p className="text-[10px] text-gray-600 truncate">{song.artist}</p>
      </div>
      <HiPlay className="text-neon text-xs opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
    </button>
  );
}
